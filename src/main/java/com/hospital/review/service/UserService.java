...
public class UserService {
    ...
    @Value("${jwt.token.secret}") // lombok value 아님
    private String secretKey;

    private long expireTimeMs = 10000 * 60 * 60L ; // 1시간
    ...
    public String login(String userName, String password) {
        ...
        User user = userRepository.findByUserName(userName).orElseThrow( // 값이 없을 경우
                () -> {throw new HospitalReviewAppException(
                        ErrorCode.USERNAME_NOT_FOUND, String.format("username %s이 없습니다", userName)
                );} );
